...
//! add cartItems localStorage start
const cartItems = document.querySelector(".header-cart-count");
cartItems.innerHTML = localStorage.getItem("cart") ? JSON.parse(localStorage.getItem("cart")).length : "0";
//! add cartItems localStorage end

//! modal dialog start
const modalDialogDOM = document.querySelector(".modal-diyalog");
const modalContentDOM = document.querySelector(".modal-diyalog .modal-content");
const btnCloseDialog = document.querySelector(".modal-diyalog .modal-close");
...
document.addEventListener("click", (e)=>{
    if(!e.composedPath().includes(modalContentDOM)){
      modalDialogDOM.classList.remove("remove");  
    }
});
